...
// --- 系統初始化 ---
// 優先使用您的設定，避免讀取到預覽環境變數
const firebaseConfig = YOUR_FIREBASE_CONFIG; 
const app = initializeApp(firebaseConfig);
const auth = getAuth(app);
const db = getFirestore(app);
// 使用固定 App ID 確保資料路徑一致
...
const AuthScreen = () => {
  ...
  // 🔥 自動清除舊的登入狀態，防止 auth/custom-token-mismatch 或 invalid-credential
  useEffect(() => {
    const clearStaleAuth = async () => {
      try {
        await signOut(auth);
        console.log("已自動清除舊的登入狀態，準備登入新專案。");
      } catch (e) {
        // Ignore error
      }
    };
  ...
  if (!auth) return <div className="min-h-screen flex items-center justify-center bg-red-50 p-4 text-red-600 font-bold">Firebase 設定錯誤</div>;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if(err.code === 'auth/weak-password') msg = "密碼強度不足 (至少 6 位)";
      if(err.code === 'auth/invalid-credential') msg = "帳號或密碼錯誤 (或帳號不存在)";
      setError(msg);
  ...
  const handleDemoLogin = async () => {
    ...
    try { await signInAnonymously(auth); } 
      ...
      setError("訪客登入失敗 (請確認 Firebase 後台已啟用匿名登入)"); 
      console.error(err);
    } 
    finally { setLoading(false); }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-100 p-4 font-sans">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl overflow-hidden p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <input type="password" placeholder="Password" value={password} onChange={e=>setPassword(e.target.value)} className="w-full border border-slate-300 p-3 rounded-lg outline-none focus:border-teal-500" required />
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const latestSession = sessions[0]; // sessions are ordered by date desc
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row font-sans text-slate-800">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 bg-slate-50">
          ...
          {viewMode === 'dashboard' && (
            <div className="space-y-6 max-w-7xl mx-auto">
              <div className="flex items-center gap-2 mb-4">
                <div className="bg-teal-100 text-teal-700 p-2 rounded-lg"><Sparkles className="w-5 h-5"/></div>
                <span className="text-sm font-bold text-slate-500">目前鎖定：<span className="text-teal-700 text-base">{dashboardStats.latestSessionName}</span></span>
              </div>
                  ...
                  <div className="flex-1 overflow-auto">
                    <table className="w-full text-left min-w-[500px]">
                      <thead className="text-slate-400 text-xs uppercase bg-slate-50 sticky top-0 z-10">
                        <tr><th className="p-3">日期</th><th className="p-3">借用人</th><th className="p-3">物品</th><th className="p-3">狀態</th></tr>
                      </thead>
          ...
          {/* Other Views (Sessions, Equipment, Loans, Categories) are identical to previous version */}
          {viewMode === 'sessions' && (
