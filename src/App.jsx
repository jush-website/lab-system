...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Recent Activity Feed (Mixing Borrow and Return Events)
    const qLoans = query(
      collection(db, 'artifacts', appId, 'public', 'data', 'loans'), 
      where('sessionId', '==', targetSessionId)
      // Removed limit here to perform client-side splitting and sorting of events
    );
      ...
      snap.forEach(doc => {
        ...
        if (data.status === 'returned' && data.returnDate) {
          events.push({
            ...
            equipmentName: data.equipmentName,
            quantity: data.quantity,
            // Use updatedAt if available, else fake it slightly after createdAt or use current time as fallback for display order
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row font-sans text-slate-800">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 bg-slate-50">
          ...
          {/* 🟢 CATEGORIES VIEW (Updated: Improved Wrapping & Layout) */}
          {viewMode === 'categories' && (
               ...
               {categories.map(c => (
                 <div key={c.id} className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 flex justify-between items-start group hover:border-teal-300 transition-colors min-h-[80px] h-auto">
                   <div className="flex items-start gap-3 overflow-hidden w-full">
                     ...
                     {/* Added break-all and whitespace-normal to fix text clipping */}
